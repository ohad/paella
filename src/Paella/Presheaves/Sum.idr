...
public export 0
FamSum : SnocList p.family -> p.family
FamSum sf w = ForAny sf $ \f => f w

||| Presheaf structure of sum presheaf
export
BoxCoalgSum : {sf : SnocList p.family} ->
...
export
(.evalSum) : {ws : SnocList p.world} -> {0 f : p.family} -> (coalg : BoxCoalg f) ->
...
export
(.currySum) : {ws : SnocList p.world} -> {0 f,g : p.family} -> (coalg : BoxCoalg f) ->
...
export
(.uncurrySum) : {ws : SnocList p.world} -> {g : p.family} -> (coalg : BoxCoalg g) ->
...
export
expSumMap : {ws : SnocList p.world} -> {0 f, g : p.family} ->
  (f -|> g) -> (f ^ ws -|> g ^ ws)
